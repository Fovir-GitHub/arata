...
/// The card footer: GitHub/Demo icon-buttons and tag chips. Shown only when
/// the project has at least one of these.
fn view_footer(project: Project) -> Element(msg) {
  let links = view_links(project.github, project.demo)
...
/// The GitHub and Demo icon-buttons, if their URLs are set.
fn view_links(
